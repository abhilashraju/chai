...
struct sock_address
{
    ...
    sock_address(std::string addr, int port)
    {
        ...
        {
            throw socket_exception(std::string("address error: ") +
                                   strerror(r));
        }
    }
};
struct sock_base
{
    ...
    friend void shutdown(sock_base& stream)
    {
    ...
    friend void listen(sock_base& stream)
    {
        ...
        {
            throw socket_exception(std::string("listen error: ") +
                                   strerror(errno));
        }
    }
    ...
    friend sock_base accept(const sock_base& stream)
    {
        ...
        if ((fd = ::accept(stream.fd_, (struct sockaddr*)&address.address,
                           (socklen_t*)&addrlen)) < 0)
        {
            throw socket_exception(std::string("accept error: ") +
                                   strerror(errno));
        }
    ...
    {
        int r = ::send(stream.fd_, buff.data(), buff.read_length(),
                       MSG_NOSIGNAL);
        if (r < 0)
        {
            throw socket_exception(strerror(r));
    ...
    {
        sock_base newstream;
        if ((newstream.fd_ = ::connect(stream.fd_,
                                       (struct sockaddr*)&serv_addr.address,
