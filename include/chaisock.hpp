...
struct sock_base {
  ...
  friend void set_blocked(sock_base &stream, bool block = true) {
    if (int r = fcntl(stream.fd_, F_SETFL,
                      fcntl(stream.fd_, F_GETFL) |
                          (block ? F_LOCK : O_NONBLOCK)) < 0) {
      throw socket_exception(std::string("block failed error: ") + strerror(r));
    }
  }
  ...
  template <typename Buffer> friend int read(sock_base &stream, Buffer &buff) {
  ...
  friend sock_base connect(sock_base &stream, const sock_address &serv_addr) {
    sock_base newstream;
    if ((newstream.fd_ =
  ...
  friend auto close(sock_base &stream) { return ::close(stream.fd_); }
};
template <typename Buffer> struct sock_stream {
  sock_base base_;
  Buffer buff;
  bool failed_{false};
  int gcount_{0};
