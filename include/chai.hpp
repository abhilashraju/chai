    ...
    return stdexec::just() | stdexec::then([=]() {
               ...
               if (!connected)
               {
                   throw std::runtime_error(
                       "Not Able to listen . Port may be in use...\n");
               }
               return listener;
           });
}
...
template <typename Handler, typename SOCK_TYPE = sock_base>
struct broadcast_handler
{
    ...
    struct ClientList
    {
        std::vector<std::unique_ptr<SOCK_TYPE>> clients;
        ...
        void add_client(SOCK_TYPE* client)
        {
            std::lock_guard<std::mutex> guard(client_mutex);
            clients.emplace_back(client);
        }
        void remove_client(SOCK_TYPE* client)
        {
            std::lock_guard<std::mutex> guard(client_mutex);
            clients.erase(
        ...
        std::optional<SOCK_TYPE*> find(int fd)
        {
            std::lock_guard<std::mutex> guard(client_mutex);
            const auto& iter = std::find_if(cbegin(clients), cend(clients),
                                            [=](const auto& sock) {
                return sock->fd() == fd;
            });
    ...
    {
        getClientList().add_client(new SOCK_TYPE(std::move(newsock)));
    }
    auto handle_read(int fd) const
    {
...
inline auto spawn_clients(auto client_agent, auto newconnection, auto do_work)
{
    struct async_context
    {
        ...
        sock_base stream;
        // stdexec::async_scope* scope{new stdexec::async_scope()};//need to
        // find a way to fix memleak;
    ...
    auto work = stdexec::just(context) | stdexec::then([=](auto context) {
...
inline auto spawn_http_clients(auto client_agent, auto newconnection,
                               auto do_work)
{
    struct async_context
    {
        ...
        exec::async_scope* scope{
            new exec::async_scope()}; // need to find a way to fix memleak;
        stdexec::in_place_stop_source stop_src;
        stdexec::in_place_stop_token token{stop_src.get_token()};
        async_context(sock_base conn) : stream(std::move(conn)) {}
    };
    auto context = new async_context(std::move(newconnection));
    auto work = stdexec::just(std::unique_ptr<async_context>(context)) |
                stdexec::let_value(
                    [=](auto& context) {
        auto child_work =
            stdexec::just(context.get()) | stdexec::then([](auto contextptr) {
            ...
            stdexec::let_value([=, contextptr = context.get()](auto& stream) {
                auto client_work = do_work(stream, contextptr->stop_src);
                return client_work;
            }) |
            stdexec::let_value([contextptr = context.get()](auto& buff) {
                string_buffer strbuff(buff);
