...
int main(int argc, char const *argv[]) {
  ...
  auto workTask =
      stdexec::schedule(net_thread.get_scheduler()) |
      wait_for_io(client, remotedata.get_buffer()) | stdexec::then([&](auto) {
  ...
  auto work=exec::repeat_effect_until(std::move(workTask)) |
   ...
   auto uiTask=stdexec::schedule(io_thread.get_scheduler()) |
            wait_for_io(io, io_data.get_buffer()) | stdexec::then([&](auto) {
